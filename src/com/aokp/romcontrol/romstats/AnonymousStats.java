...
package com.aokp.romcontrol.romstats;
...
public class AnonymousStats extends PreferenceActivity implements
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (lastCheck > 1) {
			...
			mPrefHolder = prefSet.findPreference(PREF_LAST_REPORT_ON);
			PreferenceCategory prefCat = (PreferenceCategory) prefSet.findPreference("pref_settings");
		...
		if (preference == mEnableReporting) {
			...
			if (mPersistentOptout.isChecked()) {
				try {
					...
					File dir = new File (sdCard.getAbsolutePath() + "/.EmotionROMStats");
					dir.mkdirs();
	...
	public void onClick(DialogInterface dialog, int which) {
		if (which == DialogInterface.BUTTON_POSITIVE) {
			...
			Uri uri = Uri.parse("http://emotroid.com/");
			startActivity(new Intent(Intent.ACTION_VIEW, uri));
		}
	}
}
